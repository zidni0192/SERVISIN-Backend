...
module.exports = {
    ...
    getMitraByid: (idMitra) => {
        return new Promise((resolve, reject) => {
            connection.query('SELECT tb_mitra.idMitra,tb_mitra.fullname, tb_mitra.idSubCat, tb_mitra.email, tb_mitra.nohp,tb_mitra.lat,tb_mitra.long,tb_mitra.image AS imageMitra, tb_mitra.password, tb_mitra.salt, tb_mitra.role, tb_subCategory.idSubCat, tb_subCategory.idCategory, tb_subCategory.subName, tb_subCategory.price, tb_subCategory.image AS imageSub    FROM tb_mitra INNER JOIN tb_subCategory ON tb_mitra.idSubCat = tb_subCategory.idSubCat  WHERE idMitra=?', idMitra, (err, result) => {
                console.log(idMitra);
